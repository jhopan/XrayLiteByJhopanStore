...
show_menu() {
    ...
    echo -e "  ${PURPLE}11.${NC} CloudFront Management"
...
list_cloudfront() {
    echo ""
    echo -e "${CYAN}=== CloudFront Domains ===${NC}"
    echo ""
    ...
    if [ -f "$CF_CONFIG_FILE" ]; then
        ...
        while IFS= read -r domain; do
            [ -z "$domain" ] && continue
            echo -e "  ${GREEN}$COUNTER.${NC} $domain"
            ((COUNTER++))
        done < "$CF_CONFIG_FILE"
        
        if [ $COUNTER -eq 1 ]; then
            echo "  (no CloudFront domains configured)"
        fi
    else
        echo "  (no CloudFront domains configured)"
    fi
...
add_cloudfront() {
    echo ""
    echo -e "${CYAN}=== Tambah CloudFront Domain ===${NC}"
    echo ""
    echo -e "${YELLOW}Format: dXXXXXXXXXXX.cloudfront.net${NC}"
    echo ""
    ...
    if [[ ! "$CF_DOMAIN" =~ \.cloudfront\.net$ ]]; then
        echo -e "${YELLOW}Warning: Domain tidak match format CloudFront (*.cloudfront.net)${NC}"
    ...
    # Add to config file
    echo "$CF_DOMAIN" >> "$CF_CONFIG_FILE"
    
    echo ""
    echo -e "${GREEN}✓ CloudFront domain added: $CF_DOMAIN${NC}"
    echo ""
    echo -e "${BLUE}Testing connection...${NC}"
    
    # Test connection
    if curl -s -o /dev/null -w "%{http_code}" "https://$CF_DOMAIN/health" | grep -q "200\|404"; then
        echo -e "${GREEN}✓ CloudFront reachable${NC}"
    else
        echo -e "${YELLOW}⚠ CloudFront might not be configured yet${NC}"
        echo -e "${YELLOW}  Setup origin di AWS Console: $DOMAIN${NC}"
    fi
    
    # Update nginx if needed
    if ! grep -q "*.cloudfront.net" /etc/nginx/sites-available/vpn; then
        echo ""
        echo -e "${BLUE}Updating nginx config...${NC}"
        
        # Backup
        cp /etc/nginx/sites-available/vpn /etc/nginx/sites-available/vpn.backup-cf-$(date +%Y%m%d)
        
        # Update server_name
        sed -i 's/server_name .*/& *.cloudfront.net;/' /etc/nginx/sites-available/vpn
        ...
        if nginx -t 2>/dev/null; then
            ...
            cp /etc/nginx/sites-available/vpn.backup-cf-$(date +%Y%m%d) /etc/nginx/sites-available/vpn
        fi
    fi
...
delete_cloudfront() {
    echo ""
    echo -e "${CYAN}=== Hapus CloudFront Domain ===${NC}"
    echo ""
    
    if [ ! -f "$CF_CONFIG_FILE" ] || [ ! -s "$CF_CONFIG_FILE" ]; then
        echo -e "${RED}Tidak ada CloudFront domain!${NC}"
        echo ""
        read -p "Tekan ENTER untuk kembali ke menu..."
        return
    fi
    ...
    read -p "CloudFront domain yang mau dihapus: " CF_DOMAIN
    ...
    # Check if exists
    if ! grep -q "^$CF_DOMAIN$" "$CF_CONFIG_FILE"; then
        echo -e "${RED}Error: Domain tidak ditemukan!${NC}"
        return
    fi
    
    echo ""
    read -p "Yakin mau hapus '$CF_DOMAIN'? [y/N]: " CONFIRM
    
    if [[ "$CONFIRM" =~ ^[Yy]$ ]]; then
        # Remove from config
        sed -i "/^$CF_DOMAIN$/d" "$CF_CONFIG_FILE"
        echo ""
        echo -e "${GREEN}✓ CloudFront domain deleted${NC}"
    else
        echo -e "${YELLOW}Cancelled${NC}"
    fi
...
generate_client_config() {
    echo ""
    echo -e "${CYAN}=== Generate Client Config ===${NC}"
    echo ""
    ...
    # Get UUID
    CONFIG_FILE='/usr/local/etc/xray/config.json'
    UUID=$(python3 -c "import json; config=json.load(open('$CONFIG_FILE')); clients=config['inbounds'][0]['settings']['clients']; print(next((c['id'] for c in clients if c.get('email')==\"$USERNAME\"), None))" 2>/dev/null)
    
    if [ -z "$UUID" ]; then
        echo -e "${RED}Error: User tidak ditemukan!${NC}"
        return
    fi
    
    echo ""
    echo -e "${PURPLE}═══════════════════════════════════════════════════════════════${NC}"
    echo -e "${PURPLE}Config untuk: ${GREEN}$USERNAME${NC}"
    echo -e "${PURPLE}UUID: ${GREEN}$UUID${NC}"
    echo -e "${PURPLE}═══════════════════════════════════════════════════════════════${NC}"
    echo ""
    
    # Direct config
    echo -e "${CYAN}【 1. Direct Connection (VPS) 】${NC}"
    echo ""
    echo -e "${BLUE}v2rayNG/Clash format:${NC}"
    echo "  Address: $DOMAIN"
    echo "  Port: 443"
    echo "  UUID: $UUID"
    echo "  Network: ws"
    ...
    if [ -f "$CF_CONFIG_FILE" ] && [ -s "$CF_CONFIG_FILE" ]; then
        ...
        while IFS= read -r CF_DOMAIN; do
            ...
            echo -e "${CYAN}【 $COUNTER. CloudFront ($CF_DOMAIN) 】${NC}"
            echo ""
            echo -e "${BLUE}v2rayNG/Clash format:${NC}"
            echo "  Address: $CF_DOMAIN"
            echo "  Port: 443"
            echo "  UUID: $UUID"
            echo "  Network: ws"
            echo "  Path: /vless"
    ...
    echo "  • Direct = Latency rendah (~50-80ms)"
    echo "  • CloudFront = Bypass DPI, zero-rated potential (~80-100ms)"
    echo "  • Test CloudFront dengan kuota reguler dulu sebelum pakai kuota spesial"
    echo ""
    read -p "Tekan ENTER untuk kembali ke menu..."
}
...
cloudfront_menu() {
    while true; do
        ...
        echo -e "${CYAN}═══════════════════════════════════════════════════════════════${NC}"
        echo ""
        echo -e "  ${GREEN}1.${NC} Tambah CloudFront domain"
        echo -e "  ${GREEN}2.${NC} Hapus CloudFront domain"
        echo -e "  ${GREEN}3.${NC} Generate client config (Direct + CloudFront)"
        echo -e "  ${RED}0.${NC} Kembali ke menu utama"
        echo ""
        echo -e "${CYAN}═══════════════════════════════════════════════════════════════${NC}"
        echo ""
        
        read -p "Pilihan [0-3]: " CF_CHOICE
